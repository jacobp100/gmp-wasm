...
type OutputType<T> = T extends number
  ? Rational
  : T extends Integer
  ? Rational
  : T extends Rational
...
export function getRationalContext(gmp: WasmInstance, ctx: any) {
    ...
    add<T extends AllTypes>(val: T): OutputType<T> {
      if (typeof val === "number" || isInteger(val)) {
        ...
        gmp.mpq_add(
          n.mpq_t,
          this.mpq_t,
          RationalFn(val as number | Integer).mpq_t
        );
  ...
  const parseParameters = (
    mpq_t: number,
    p1: string | number | Rational | Integer,
    p2?: string | number | Integer
  ) => {
    ...
    const finalString =
      p2 !== undefined ? `${p1.toString()}/${p2.toString()}` : p1.toString();
  ...
  const RationalFn = (
    ...
    registry.register(instance, instance.mpq_t);

    return instance;
  };

  const registry = new FinalizationRegistry((mpq_t: number) => {
    gmp.mpq_clear(mpq_t);
